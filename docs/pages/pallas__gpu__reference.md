...
- [ .md](../../_sources/pallas/gpu/reference.md "Download source file")
-  .pdf
...
- [What is a GPU?](#what-is-a-gpu)
  - [In-order execution & using multiple hardware units](#in-order-execution-using-multiple-hardware-units)
  - [Memory spaces](#memory-spaces)
    - [Requesting/allocating memory in specific memory spaces](#requesting-allocating-memory-in-specific-memory-spaces)
    - [Taking advantage of the L2 cache](#taking-advantage-of-the-l2-cache)
- [Array layouts and memory reference transforms](#array-layouts-and-memory-reference-transforms)
  - [Memory reference transforms](#memory-reference-transforms)
  - [Array layouts](#array-layouts)
- [MMA (TensorCore)](#mma-tensorcore)
  - [Memory space of `A` and `B` operands](#memory-space-of-a-and-b-operands)
  - [Transposed operands](#transposed-operands)
  - [Hopper (`wgmma`)](#hopper-wgmma)
    - [Allocating the accumulator](#allocating-the-accumulator)
    - [Preparing the `A` and `B` operands](#preparing-the-a-and-b-operands)
    - [Issuing the operation](#issuing-the-operation)
    - [Waiting for the operation to complete](#waiting-for-the-operation-to-complete)
  - [Blackwell (`tcgen05`)](#blackwell-tcgen05)
    - [Allocating the accumulator / Using TMEM](#allocating-the-accumulator-using-tmem)
      - [Loads](#loads)
      - [Stores](#stores)
    - [Preparing the `A` and `B` operands](#id1)
    - [Issuing the operation](#id2)
    - [Waiting for the operation to complete](#id3)
    - [Collective MMA](#collective-mma)
- [Using `core_map`](#using-core-map)
  - [Replacing `pl.pallas_call` with `pl.core_map` or `plgpu.kernel`](#replacing-pl-pallas-call-with-pl-core-map-or-plgpu-kernel)
  - [Using multiple Pallas threads per CUDA block](#using-multiple-pallas-threads-per-cuda-block)
  - [Using CUDA block clusters](#using-cuda-block-clusters)
  - [Collective allocations in `pl.run_scoped`](#collective-allocations-in-pl-run-scoped)
  - [Global (grid-wide) allocations using `pl.get_global`](#global-grid-wide-allocations-using-pl-get-global)
- [Synchronization structures and primitives](#synchronization-structures-and-primitives)
  - [`commit_smem`](#commit-smem)
  - [`Barrier`](#barrier)
    - [Asynchronous GMEM-to-SMEM copies](#asynchronous-gmem-to-smem-copies)
    - [Explicit arrival (cross-thread synchronization)](#explicit-arrival-cross-thread-synchronization)
    - [Awaiting `tcgen05` TensorCore instructions](#awaiting-tcgen05-tensorcore-instructions)
  - [`ClusterBarrier`](#clusterbarrier)
    - [Reusing SMEM for collective async copies](#reusing-smem-for-collective-async-copies)
    - [Reusing TMEM for collective MMAs on Blackwell](#reusing-tmem-for-collective-mmas-on-blackwell)
  - [`Semaphore`](#semaphore)
- [Cluster launch control](#cluster-launch-control)
  - [Directly using the cluster launch control API](#directly-using-the-cluster-launch-control-api)
  - [Using the `plgpu.dynamic_scheduling_loop` helper](#using-the-plgpu-dynamic-scheduling-loop-helper)
- [Asynchronous copies](#asynchronous-copies)
  - [GMEM to SMEM copies](#gmem-to-smem-copies)
    - [Collective copies](#collective-copies)
    - [Leader-tracked collective copies (Blackwell only)](#leader-tracked-collective-copies-blackwell-only)
      - [`CopyPartition.PARTITIONED(axis)`](#copypartition-partitioned-axis)
      - [`CopyPartition.REPLICATED`](#copypartition-replicated)
  - [SMEM to GMEM copies](#smem-to-gmem-copies)
    - [Only awaiting the read from SMEM](#only-awaiting-the-read-from-smem)
    - [Grouping multiple copies](#grouping-multiple-copies)
  - [Asynchronous gathers](#asynchronous-gathers)
  - [NVLINK transfers](#nvlink-transfers)
- [Inline Mosaic GPU](#inline-mosaic-gpu)
- [Compiler parameters](#compiler-parameters)
- [Debugging](#debugging)
- [Calling kernels from PyTorch](#calling-kernels-from-pytorch)

# Writing Mosaic GPU kernels with Pallas[\#](#writing-mosaic-gpu-kernels-with-pallas "Link to this heading")
...
## What is a GPU?[\#](#what-is-a-gpu "Link to this heading")
...
![A diagram of one NVIDIA SM](../../_static/pallas/gpu/nvidia_sm.svg)
...
This is very similar to a programming model popularized by [Triton](https://triton-lang.org/), but as you will see there are a few differences. Mosaic GPU tends to be more low level, which usually means you will have to put in more work, but it also puts you more in control. In our view both approaches have their merits and we encourage you to pick the backend that suits your needs the best! Pallas supports and will continue to support Triton as an alternative GPU backend.

### In-order execution & using multiple hardware units[\#](#in-order-execution-using-multiple-hardware-units "Link to this heading")
...
To extend this even further, we can take advantage of this hardware-unit-level parallelism by allowing multiple Pallas threads to run concurrently. If one of the threads primarily occupies the ALU, while another one primarily issues TensorCore related instructions, we can take advantage of the efficient context switching built into the warp schedulers to keep both units busy. This is one of the core idea behind algorithms such as [FlashAttention 3](https://arxiv.org/abs/2407.08608) or [CUTLASS ping-pong matmul kernels](https://pytorch.org/blog/cutlass-ping-pong-gemm-kernel/).

For more information on how warp scheduling and instruction issue works, we recommend reading [Analyzing Modern NVIDIA GPU cores](https://arxiv.org/abs/2503.20481).

### Memory spaces[\#](#memory-spaces "Link to this heading")
...
![A diagram of memory spaces of an NVIDIA GPU](../../_static/pallas/gpu/memory_spaces.svg)
...
#### Requesting/allocating memory in specific memory spaces[\#](#requesting-allocating-memory-in-specific-memory-spaces "Link to this heading")
...
#### Taking advantage of the L2 cache[\#](#taking-advantage-of-the-l2-cache "Link to this heading")
...
## Array layouts and memory reference transforms[\#](#array-layouts-and-memory-reference-transforms "Link to this heading")
...
### Memory reference transforms[\#](#memory-reference-transforms "Link to this heading")
...
- `plgpu.SwizzleTransform(swizzle_in_bytes)`, which transforms the data as described in the [PTX docs](https://docs.nvidia.com/cuda/parallel-thread-execution/#tensor-swizzling-modes) and [CUDA docs](https://docs.nvidia.com/cuda/cuda-c-programming-guide/#the-swizzle-modes). Swizzling is useful, because it allows transferring data in MMA-related layouts between register and shared memory without bank conflicts. The exact details of how the memory looks like after swizzling *are not that important*, since all primitives will account for it automatically. Note that the swizzle amount is specified in bytes (only 128, 64, 32 and 16 are supported), and is usually accompanied by a `TileTransform` (which uses elements in its shape!).
...
When performing GMEM-SMEM or SMEM-GMEM copies and a `plgpu.TileTransform` is applied on the SMEM reference, the offsets into the GMEM reference must be aligned with the tile sizes. If that is not the case, the transfer may produce wrong results.

### Array layouts[\#](#array-layouts "Link to this heading")

There are a few useful layouts we have defined for you so far:

- `plgpu.Layout.WGMMA`, which is the layout in which the Hopper-generation TensorCore expects the MMA accumulator or 16-bit input operands to have in registers. Calling `plgpu.Layout.WGMMA.reduce(axes)` gives a layout suitable for values reduced along the specified axes, e.g. `reduce(1)` for a row result and `reduce(0)` for a column result. Re-broadcasting the reduced dimensions is free and produces a value with `WGMMA` layout.
...
## MMA (TensorCore)[\#](#mma-tensorcore "Link to this heading")
...
### Memory space of `A` and `B` operands[\#](#memory-space-of-a-and-b-operands "Link to this heading")
...
### Transposed operands[\#](#transposed-operands "Link to this heading")
...
### Hopper (`wgmma`)[\#](#hopper-wgmma "Link to this heading")

In this section, we cover the basics of using the Hopper-generation TensorCores, exposed in PTX as the [`wgmma.mma_async` instruction](https://docs.nvidia.com/cuda/parallel-thread-execution/#asynchronous-warpgroup-level-matrix-instructions-wgmma-mma).

#### Allocating the accumulator[\#](#allocating-the-accumulator "Link to this heading")
...
#### Preparing the `A` and `B` operands[\#](#preparing-the-a-and-b-operands "Link to this heading")
...
#### Issuing the operation[\#](#issuing-the-operation "Link to this heading")
...
#### Waiting for the operation to complete[\#](#waiting-for-the-operation-to-complete "Link to this heading")
...
### Blackwell (`tcgen05`)[\#](#blackwell-tcgen05 "Link to this heading")

The Blackwell generation has significantly redesigned the TensorCore subunit. It is now significantly more independent from the regular warp schedulers and no longer uses or even supports using registers as its operands. In their place, a new memory space called *tensor memory* (TMEM) has been introduced. What’s more TensorCores from pairs of SMs can now pool their resources and compute larger MMA operations that span both SMs. We call this a [“collective MMA operation”](#collective-mma).

#### Allocating the accumulator / Using TMEM[\#](#allocating-the-accumulator-using-tmem "Link to this heading")

TMEM references can be allocated in the same way in which all other references are allocated—using [`pl.run_scoped`](../../_autosummary/jax.experimental.pallas.run_scoped.html#jax.experimental.pallas.run_scoped "jax.experimental.pallas.run_scoped"):
...
##### Loads[\#](#loads "Link to this heading")

Loads can be performed using [`plgpu.async_load_tmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.async_load_tmem.html#jax.experimental.pallas.mosaic_gpu.async_load_tmem "jax.experimental.pallas.mosaic_gpu.async_load_tmem") and awaited using [`plgpu.wait_load_tmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.wait_load_tmem.html#jax.experimental.pallas.mosaic_gpu.wait_load_tmem "jax.experimental.pallas.mosaic_gpu.wait_load_tmem"):
...
##### Stores[\#](#stores "Link to this heading")

Conversely, stores are performed using [`plgpu.async_store_tmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.async_store_tmem.html#jax.experimental.pallas.mosaic_gpu.async_store_tmem "jax.experimental.pallas.mosaic_gpu.async_store_tmem") and awaited using [`plgpu.commit_tmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.commit_tmem.html#jax.experimental.pallas.mosaic_gpu.commit_tmem "jax.experimental.pallas.mosaic_gpu.commit_tmem"):

    plgpu.async_store_tmem(tmem_ref, smem_ref[...])
    plgpu.commit_tmem()
    smem_ref2[...] = plgpu.async_load_tmem(tmem_ref)  # Safe to read from tmem_ref now

#### Preparing the `A` and `B` operands[\#](#id1 "Link to this heading")

We recommend passing in `A` and `B` through shared memory. In this case the [correct tiling and swizzling transforms must be specified](#memory-space-a-b-mma). The `A` operand can be passed in as a TMEM reference as well, but it must be packed.

#### Issuing the operation[\#](#id2 "Link to this heading")
...
#### Waiting for the operation to complete[\#](#id3 "Link to this heading")

Awaiting the result of a [`plgpu.tcgen05_mma`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.tcgen05_mma.html#jax.experimental.pallas.mosaic_gpu.tcgen05_mma "jax.experimental.pallas.mosaic_gpu.tcgen05_mma") call requires the use of a `Barrier`. We recommend reading through the reference documentation for [`Barrier`s](#barrier), and especially its [Blackwell-related subsection](#awaiting-tcgen05-instructions) for more information.

If the barrier is passed in directly to the [`plgpu.tcgen05_mma`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.tcgen05_mma.html#jax.experimental.pallas.mosaic_gpu.tcgen05_mma "jax.experimental.pallas.mosaic_gpu.tcgen05_mma"), completing a wait on that barrier will indicate that the final accumulator has been written to TMEM. For example:
...
If no barrier is given to [`plgpu.tcgen05_mma`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.tcgen05_mma.html#jax.experimental.pallas.mosaic_gpu.tcgen05_mma "jax.experimental.pallas.mosaic_gpu.tcgen05_mma"), its completion will be tracked only once `plgpu.tcgen05_commit` is called:
...
#### Collective MMA[\#](#collective-mma "Link to this heading")
...
![A diagram showing the partitioning of operands in a collective MMA](../../_static/pallas/gpu/collective_mma.svg)
...
To make loading of the `B` operand easier, [`plgpu.copy_gmem_to_smem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem.html#jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem "jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem") can be used together with `collective_axes` and `leader_tracked=plgpu.CopyPartition.PARTITIONED(axis)` to indicate that the two Pallas threads along the collective axis should load the same slice, but each will only obtain half of it. Unlike a copy with `collective_axes` alone it does not utilize TMA multicast (since each thread loads a distinct slice of data), but it can simplify the indexing logic a bit.
...
## Using `core_map`[\#](#using-core-map "Link to this heading")
...
### Replacing `pl.pallas_call` with `pl.core_map` or `plgpu.kernel`[\#](#replacing-pl-pallas-call-with-pl-core-map-or-plgpu-kernel "Link to this heading")
...
We can write a similar kernel using `pl.core_map`. One big difference is that unlike `pl.pallas_call`, no GMEM\<-\>SMEM copies will be inserted automatically. If you want them, you can either insert them yourself or use the [`plgpu.emit_pipeline`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.emit_pipeline.html#jax.experimental.pallas.mosaic_gpu.emit_pipeline "jax.experimental.pallas.mosaic_gpu.emit_pipeline") helper. We recommend reviewing the [software pipelining guide](pipelining.html).
...
### Using multiple Pallas threads per CUDA block[\#](#using-multiple-pallas-threads-per-cuda-block "Link to this heading")
...
While this example is simple, you can find a more complicated example in the [synchronization section](#cross-thread-synchronization).
...
### Using CUDA block clusters[\#](#using-cuda-block-clusters "Link to this heading")
...
### Collective allocations in `pl.run_scoped`[\#](#collective-allocations-in-pl-run-scoped "Link to this heading")
...
### Global (grid-wide) allocations using `pl.get_global`[\#](#global-grid-wide-allocations-using-pl-get-global "Link to this heading")

Sometimes, it is useful to allocate [semaphores](#semaphore) in a way that enables them to be shared by all the parallel program instances. For example, when the number of parallel instances is small enough that the kernel is persistent. Such allocations are possible using `pl.get_global`:
...
## Synchronization structures and primitives[\#](#synchronization-structures-and-primitives "Link to this heading")
...
### `commit_smem`[\#](#commit-smem "Link to this heading")
...
### `Barrier`[\#](#barrier "Link to this heading")

This is essentially a thin wrapper around an array of PTX `mbarrier` types and is passed in as a reference. All functions involving barriers expect to only get a single barrier argument, and so if the reference contains multiple, you have to extract one of them explicitly using `barriers.at[index]`. `Barrier`s are always allocated in SMEM and as such have relatively low overheads. Each barrier can be configured to complete after a fixed number of “arrivals” (by default 1).
...
#### Asynchronous GMEM-to-SMEM copies[\#](#asynchronous-gmem-to-smem-copies "Link to this heading")
...
#### Explicit arrival (cross-thread synchronization)[\#](#explicit-arrival-cross-thread-synchronization "Link to this heading")

Any thread can explicitly arrival on a barrier using the following function:

    plgpu.barrier_arrive(barrier)

This is especially useful when synchronizing two threads that are in producer/consumer roles. In this case, we recommend allocating two arrays of `Barrier`s, with size equal to the size of the “queue” used to pass data between the two threads. For example, assume one thread continues writing tiles of an array to SMEM while another thread reads them. We triple-buffer the SMEM region to allow more asynchrony between the two threads:
...
#### Awaiting `tcgen05` TensorCore instructions[\#](#awaiting-tcgen05-tensorcore-instructions "Link to this heading")
...
If you mean to use `Barrier`s to indicate to other threads that a TensorCore operation is complete, allocate the barrier with `orders_tensor_core=True`. This argument will insert the necessary instructions to prevent the problematic reordering mentioned above.

Unlike in older GPUs, the only way to observe the completion of Blackwell-generation TensorCore instructions is to pass in a `Barrier` reference to the [`plgpu.tcgen05_mma`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.tcgen05_mma.html#jax.experimental.pallas.mosaic_gpu.tcgen05_mma "jax.experimental.pallas.mosaic_gpu.tcgen05_mma") function. Once the MMA is complete, the TensorCore will arrive on the barrier.

Note that this use of `Barrier`s requires that they are created with `orders_tensor_core=True`, since they are used to synchronize with TensorCore operations.
...
### `ClusterBarrier`[\#](#clusterbarrier "Link to this heading")
...
#### Reusing SMEM for collective async copies[\#](#reusing-smem-for-collective-async-copies "Link to this heading")
...
#### Reusing TMEM for collective MMAs on Blackwell[\#](#reusing-tmem-for-collective-mmas-on-blackwell "Link to this heading")
...
### `Semaphore`[\#](#semaphore "Link to this heading")
...
- [`pl.semaphore_signal`](../../_autosummary/jax.experimental.pallas.semaphore_signal.html#jax.experimental.pallas.semaphore_signal "jax.experimental.pallas.semaphore_signal"), which atomically increments the semaphore. Any effects performed by the thread before the signal (including reads or writes to remote memory over NVLINK) are guaranteed to complete before the signal is visible on the target device.

- [`pl.semaphore_wait`](../../_autosummary/jax.experimental.pallas.semaphore_wait.html#jax.experimental.pallas.semaphore_wait "jax.experimental.pallas.semaphore_wait"), which blocks the thread until the semaphore reaches *at least* the desired value, at which point the value is atomically decreased and the thread is awoken. The function can be optionally called with `decrement=False`, which will wake the thread as soon as the value is at least the requested value, but the value of the semaphore will not be decreased. The non-decrementing version is a bit more efficient.
...
## Cluster launch control[\#](#cluster-launch-control "Link to this heading")
...
### Directly using the cluster launch control API[\#](#directly-using-the-cluster-launch-control-api "Link to this heading")

Mosaic GPU directly exposes the low-level cluster launch control API as two functions: [`plgpu.try_cluster_cancel`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.try_cluster_cancel.html#jax.experimental.pallas.mosaic_gpu.try_cluster_cancel "jax.experimental.pallas.mosaic_gpu.try_cluster_cancel") and [`plgpu.query_cluster_cancel`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.query_cluster_cancel.html#jax.experimental.pallas.mosaic_gpu.query_cluster_cancel "jax.experimental.pallas.mosaic_gpu.query_cluster_cancel"). `try_cluster_cancel` is an asynchronous operation that will atomically attempt to cancel the launch of an available block, and place the result in a Ref. The result Ref should be a scratch Ref allocated via `plgpu.TryClusterCancelResult()` (which under the hood is a 16-byte SMEM Ref). `query_cluster_cancel` will read the result and return two values: a tuple containing the indices of the grid axes that were requested, and a boolean indicating whether the cancellation was successful. If `query_cluster_cancel` was not successful, then the result of the grid indices is undefined and should not be used.
...
It is important to ensure proper synchronization on all threads throughout the cluster. In most cases when canceling multiple blocks, you may need to double-buffer the result and barrier to ensure no race conditions occur. For this reason we recommend using the [`plgpu.dynamic_scheduling_loop`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop.html#jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop "jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop") helper function.

### Using the `plgpu.dynamic_scheduling_loop` helper[\#](#using-the-plgpu-dynamic-scheduling-loop-helper "Link to this heading")

A common pattern when using dynamic work scheduling is to continuously poll and execute work within the kernel body until there are no more work left, and then exit the kernel. The [`plgpu.dynamic_scheduling_loop`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop.html#jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop "jax.experimental.pallas.mosaic_gpu.dynamic_scheduling_loop") helper function implements exactly this pattern.
...
### GMEM to SMEM copies[\#](#gmem-to-smem-copies "Link to this heading")

To schedule an asynchronous GMEM to SMEM copy, use [`plgpu.copy_gmem_to_smem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem.html#jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem "jax.experimental.pallas.mosaic_gpu.copy_gmem_to_smem"). The function takes three operands: a source ref, a destination ref and a `Barrier`. Once the copy is complete, a single arrival will be observed on the barrier, as if `plgpu.barrier_arrive(barrier)` was called by a background thread:
...
#### Collective copies[\#](#collective-copies "Link to this heading")
...
#### Leader-tracked collective copies (Blackwell only)[\#](#leader-tracked-collective-copies-blackwell-only "Link to this heading")

In the Blackwell generation, collective copies that involve clusters of two blocks support a `leader_tracked` argument. When specified, only the barrier in the first block (the “leader”) will receive arrivals once the copy completes. The barrier argument in the second block is ignored and the second block cannot use it to await the completion of the transfer.
...
##### `CopyPartition.PARTITIONED(axis)`[\#](#copypartition-partitioned-axis "Link to this heading")

When `leader_tracked=plgpu.CopyPartition.PARTITIONED(axis)`, the GMEM reference is expected to be double the size of the destination SMEM reference along the specified dimension. The destination in the first block will be overwritten with the first half of the GMEM ref, while the second block will receive the second half.

This by itself would be equivalent to performing two non-collective copies on different input slices, but the key difference is in the barrier tracking behavior described above.
...
##### `CopyPartition.REPLICATED`[\#](#copypartition-replicated "Link to this heading")

When `leader_tracked=plgpu.CopyPartition.REPLICATED`, both blocks load the same data (just like a regular collective copy), but the barrier tracking is still leader-only. This is especially useful for the scales operands of collective MMA that need to be replicated in both blocks.
...
### SMEM to GMEM copies[\#](#smem-to-gmem-copies "Link to this heading")

To schedule an asynchronous GMEM to SMEM copy, use [`plgpu.copy_smem_to_gmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.copy_smem_to_gmem.html#jax.experimental.pallas.mosaic_gpu.copy_smem_to_gmem "jax.experimental.pallas.mosaic_gpu.copy_smem_to_gmem"). As opposed to the other direction, this primitive only takes in the source and destination references. To await the completion of the copy, use the [`plgpu.wait_smem_to_gmem`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.wait_smem_to_gmem.html#jax.experimental.pallas.mosaic_gpu.wait_smem_to_gmem "jax.experimental.pallas.mosaic_gpu.wait_smem_to_gmem").
...
#### Only awaiting the read from SMEM[\#](#only-awaiting-the-read-from-smem "Link to this heading")
...
#### Grouping multiple copies[\#](#grouping-multiple-copies "Link to this heading")
...
### Asynchronous gathers[\#](#asynchronous-gathers "Link to this heading")
...
### NVLINK transfers[\#](#nvlink-transfers "Link to this heading")
...
## Inline Mosaic GPU[\#](#inline-mosaic-gpu "Link to this heading")

TODO

## Compiler parameters[\#](#compiler-parameters "Link to this heading")
...
## Calling kernels from PyTorch[\#](#calling-kernels-from-pytorch "Link to this heading")

The [`plgpu.as_torch_kernel`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.as_torch_kernel.html#jax.experimental.pallas.mosaic_gpu.as_torch_kernel "jax.experimental.pallas.mosaic_gpu.as_torch_kernel") decorator wraps a Pallas:MGPU kernel to allow invoking it with PyTorch tensors. It accepts CUDA tensors as inputs and returns newly allocated CUDA tensors on the same device.
...
`plgpu.as_torch_kernel` only supports functions that contain a single kernel invocation (e.g. via `pl.pallas_call` or `plgpu.kernel`), and no calls to other JAX operations, e.g. from [`jax.numpy`](../../jax.numpy.html#module-jax.numpy "jax.numpy").
