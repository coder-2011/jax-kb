...
What remains is to actually turn the kernel into a function that can be called with JAX arrays. We use [`plgpu.kernel`](../../_autosummary/jax.experimental.pallas.mosaic_gpu.kernel.html#jax.experimental.pallas.mosaic_gpu.kernel "jax.experimental.pallas.mosaic_gpu.kernel") for that. The grid is for now simply 2D and iterates over the output tiles. We allocate intermediates used by the kernel:
...
[](collective_matmul.html "next page")
