...
- [ .md](../_sources/jep/28845-stateful-rng.md "Download source file")
-  .pdf
...
- [API Design](#api-design)
  - [Interaction with `vmap` and `shard_map`](#interaction-with-vmap-and-shard-map)
- [Statistical Considerations](#statistical-considerations)
- [Advantages](#advantages)
- [Limitations](#limitations)
- [Evaluation](#evaluation)

# JEP 28845: Stateful Randomness in JAX[\#](#jep-28845-stateful-randomness-in-jax "Link to this heading")

[@jakevdp](http://github.com/jakevdp), *November 2025*

This document explores the addition of an **optional** stateful pseudo-random number generator (PRNG) for use in JAX; this is meant to be used alongside the classic functional PRNGs described in [Pseudorandom numbers](../random-numbers.html#pseudorandom-numbers) in cases where statefulness is convenient.

## Background[\#](#background "Link to this heading")

JAX has always required users to explicitly manage random state as part of its functional programming paradigm (see [JAX PRNG Design](263-prng.html#prng-design-jep) for background on this). Although well-motivated, this is a frequently encountered [sharp bit](https://docs.jax.dev/en/latest/notebooks/Common_Gotchas_in_JAX.html#random-numbers) for new users who are accustomed to stateful pseudorandom number APIs.
...
This JAX Enhancement Proposal (or [JEP](https://docs.jax.dev/en/latest/jep/index.html)) proposes the introduction of a Stateful PRNG API into [`jax.experimental.random`](../jax.experimental.random.html#module-jax.experimental.random "jax.experimental.random"), with a goal of eventual inclusino into [`jax.random`](../jax.random.html#module-jax.random "jax.random") itself.

## API Design[\#](#api-design "Link to this heading")

To align with best practices developed within the larger numerical Python community, we propose for the stateful PRNG API to align with To align with NumPy’s most recent PRNG API iteration, found in [`numpy.random.Generator`](https://numpy.org/doc/stable/reference/random/generator.html#numpy.random.Generator "(in NumPy v2.4)"), and typically created using the [`numpy.random.default_rng()`](https://numpy.org/doc/stable/reference/random/generator.html#numpy.random.default_rng "(in NumPy v2.4)") function. A full draft of the proposed implementation can be found at [\#28845](https://github.com/jax-ml/jax/issues/28845), but here we summarize the main features of the implementation.

A simplified version of the stateful PRNG Generator code looks like this (function and argument names follow the [`numpy.random`](https://numpy.org/doc/stable/reference/random/index.html#module-numpy.random "(in NumPy v2.4)") APIs):
...
With this implementation exposed in the [`jax.experimental.random`](../jax.experimental.random.html#module-jax.experimental.random "jax.experimental.random") namespace, usage is virtually identical to that of [`numpy.random.default_rng()`](https://numpy.org/doc/stable/reference/random/generator.html#numpy.random.default_rng "(in NumPy v2.4)"):
...
Because the statefulness in [`jax.experimental.random.StatefulPRNG`](../_autosummary/jax.experimental.random.StatefulPRNG.html#jax.experimental.random.StatefulPRNG "jax.experimental.random.StatefulPRNG") is tracked via mutable refs, the random state will correctly update even if the generator is used in transformations like [`jax.jit()`](../_autosummary/jax.jit.html#jax.jit "jax.jit"), which typically require pure functional semantics.

### Interaction with `vmap` and `shard_map`[\#](#interaction-with-vmap-and-shard-map "Link to this heading")
...
## Statistical Considerations[\#](#statistical-considerations "Link to this heading")
...
Our judgment is that the advantages of the stateful PRNG API potentially outweigh the limitations, and that we should introduce a new experimental [`stateful_rng()`](../_autosummary/jax.experimental.random.stateful_rng.html#jax.experimental.random.stateful_rng "jax.experimental.random.stateful_rng") API in the [`jax.experimental.random`](../jax.experimental.random.html#module-jax.experimental.random "jax.experimental.random") module for now. Once we get a feel for the usefulness of this, we may evenutally graduate this API to the [`jax.random`](../jax.random.html#module-jax.random "jax.random") module, perhaps with a `default_rng` alias in `jax.numpy.random`.

[](28661-jax-array-protocol.html "previous page")

previous

JEP 28661: Supporting the `__jax_array__` protocol

[](../internals/index.html "next page")
