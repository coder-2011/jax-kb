...
- [ .md](../_sources/export/shape_poly.md "Download source file")
-  .pdf
...
- [Correctness of shape polymorphism](#correctness-of-shape-polymorphism)
- [Computing with dimension variables](#computing-with-dimension-variables)
  - [Errors in presence of shape polymorphism](#errors-in-presence-of-shape-polymorphism)
  - [Comparison of symbolic dimensions is partially supported](#comparison-of-symbolic-dimensions-is-partially-supported)
    - [User-specified symbolic constraints](#user-specified-symbolic-constraints)
    - [Symbolic dimension scopes](#symbolic-dimension-scopes)
  - [Caveat for equality comparisons](#caveat-for-equality-comparisons)
  - [Dimension variables must be solvable from the input shapes](#dimension-variables-must-be-solvable-from-the-input-shapes)
  - [Shape assertion errors](#shape-assertion-errors)
- [Debugging](#debugging)

# Shape polymorphism[\#](#shape-polymorphism "Link to this heading")
...
The [`jax.export.symbolic_shape()`](../_autosummary/jax.export.symbolic_shape.html#jax.export.symbolic_shape "jax.export.symbolic_shape") is used in the above example to parse a string representation of a symbolic shape into dimension expressions objects (of type `_DimExpr`) that are usable in place of integer constants to construct shapes. The dimension expression objects overload most integer operators, so you can use them as you’d use integer constants in most cases. See [Computing with dimension variables](#computing-with-dimension-variables) for more details.

Additionally, we provide the [`jax.export.symbolic_args_specs()`](../_autosummary/jax.export.symbolic_args_specs.html#jax.export.symbolic_args_specs "jax.export.symbolic_args_specs") that can be used to construct pytrees of `jax.ShapeDtypeStruct` objects based on a polymorphic shape specification:
...
Note how the polymorphic shape specification `"a,`` ``..."` contains the placeholder `...` to be filled from the concrete shapes of the concrete shapes of the arguments `(x,`` ``y)`. The placeholder `...` stands for 0 or more dimensions, while the placeholder `_` stands for one dimension. The [`jax.export.symbolic_args_specs()`](../_autosummary/jax.export.symbolic_args_specs.html#jax.export.symbolic_args_specs "jax.export.symbolic_args_specs") supports pytrees of arguments, which are used to fill-in the dtypes and any placeholders. The function will construct a pytree of argument specifications ([`jax.ShapeDtypeStruct`](../_autosummary/jax.ShapeDtypeStruct.html#jax.ShapeDtypeStruct "jax.ShapeDtypeStruct")) matching the structure of the arguments passed to it. The polymorphic shapes specification can be a pytree prefix in cases where one specification should apply to multiple arguments, as in the above example. See [how optional parameters are matched to arguments](https://docs.jax.dev/en/latest/pytrees.html#applying-optional-parameters-to-pytrees).
...
## Correctness of shape polymorphism[\#](#correctness-of-shape-polymorphism "Link to this heading")
...
## Computing with dimension variables[\#](#computing-with-dimension-variables "Link to this heading")
...
### Errors in presence of shape polymorphism[\#](#errors-in-presence-of-shape-polymorphism "Link to this heading")
...
### Comparison of symbolic dimensions is partially supported[\#](#comparison-of-symbolic-dimensions-is-partially-supported "Link to this heading")
...
- equality is supported with a caveat: if the two symbolic dimensions denote the same value under all valuations for dimension variables, then equality evaluates to `True`, e.g., for `b`` ``+`` ``b`` ``==`` ``2*b`; otherwise the equality evaluates to `False`. See [below](#caveat-for-equality-comparisons) for a discussion of important consequences of this behavior.
...
#### User-specified symbolic constraints[\#](#user-specified-symbolic-constraints "Link to this heading")
...
Such implicit symbolic constraints are used for deciding comparisons and are checked at compile time, as explained [below](#shape-assertion-errors).
...
Just like the implicit constraints, the explicit symbolic constraints are checked at compile time, using the same mechanism as explained [below](#shape-assertion-errors).

#### Symbolic dimension scopes[\#](#symbolic-dimension-scopes "Link to this heading")

The symbolic constraints are stored in αn [`jax.export.SymbolicScope`](../_autosummary/jax.export.SymbolicScope.html#jax.export.SymbolicScope "jax.export.SymbolicScope") object, which is created implicitly for each call to `jax.export.symbolic_shapes()`. You must be careful to not mix symbolic expressions that use different scopes. For example, the following code will fail because `a1` and `a2` use different scopes (created by different invocations of [`jax.export.symbolic_shape()`](../_autosummary/jax.export.symbolic_shape.html#jax.export.symbolic_shape "jax.export.symbolic_shape")):
...
The symbolic expressions that originate from a single call to [`jax.export.symbolic_shape()`](../_autosummary/jax.export.symbolic_shape.html#jax.export.symbolic_shape "jax.export.symbolic_shape") share a scope and can be mixed up in arithmetic operations. The result would also share the same scope.
...
### Caveat for equality comparisons[\#](#caveat-for-equality-comparisons "Link to this heading")
...
### Dimension variables must be solvable from the input shapes[\#](#dimension-variables-must-be-solvable-from-the-input-shapes "Link to this heading")
...
### Shape assertion errors[\#](#shape-assertion-errors "Link to this heading")
...
First, see the [Debugging](export.html#export-debugging) documentation. Additionally, you can debug the shape refinement, which is invoked at compilation time for modules that have dimension variables or multi-platform support.
