...
- [ .md](../_sources/jep/9263-typed-keys.md "Download source file")
-  .pdf
...
# JEP 9263: Typed keys & pluggable RNGs[\#](#jep-9263-typed-keys-pluggable-rngs "Link to this heading")
...
For now, old-style RNG keys can still be created with [`jax.random.PRNGKey()`](../_autosummary/jax.random.PRNGKey.html#jax.random.PRNGKey "jax.random.PRNGKey"):
...
Starting now, new-style RNG keys can be created with [`jax.random.key()`](../_autosummary/jax.random.key.html#jax.random.key "jax.random.key"):
...
This (scalar-shaped) array behaves the same as any other JAX array, except that its element type is a key (and associated metadata). We can make non-scalar key arrays as well, for example by applying [`jax.vmap()`](../_autosummary/jax.vmap.html#jax.vmap "jax.vmap") to [`jax.random.key()`](../_autosummary/jax.random.key.html#jax.random.key "jax.random.key"):
...
Aside from switching to a new constructor, most PRNG-related code should continue to work as expected. You can continue to use keys in [`jax.random`](../jax.random.html#module-jax.random "jax.random") APIs as before; for example:
...
If for some reason you need to recover the underlying buffer (the old-style key), you can do so with [`jax.random.key_data()`](../_autosummary/jax.random.key_data.html#jax.random.key_data "jax.random.key_data"):

    >>> jax.random.key_data(key)
    Array([0, 0], dtype=uint32)

For old-style keys, [`key_data()`](../_autosummary/jax.random.key_data.html#jax.random.key_data "jax.random.key_data") is an identity operation.

## What does this mean for users?[\#](#what-does-this-mean-for-users "Link to this heading")
...
- If your code performs unsafe/unsupported operations on keys (such as indexing, arithmetic, transposition, etc; see Type Safety section below), this change will catch it. You can update your code to avoid such unsupported operations, or use [`jax.random.key_data()`](../_autosummary/jax.random.key_data.html#jax.random.key_data "jax.random.key_data") and [`jax.random.wrap_key_data()`](../_autosummary/jax.random.wrap_key_data.html#jax.random.wrap_key_data "jax.random.wrap_key_data") to manipulate raw key buffers in an unsafe way.
...
At some point in the future, we plan to deprecate [`jax.random.PRNGKey()`](../_autosummary/jax.random.PRNGKey.html#jax.random.PRNGKey "jax.random.PRNGKey") and require the use of [`jax.random.key()`](../_autosummary/jax.random.key.html#jax.random.key "jax.random.key").

### Detecting new-style typed keys[\#](#detecting-new-style-typed-keys "Link to this heading")
...
### Type annotations for PRNG Keys[\#](#type-annotations-for-prng-keys "Link to this heading")
...
### Notes for JAX library authors[\#](#notes-for-jax-library-authors "Link to this heading")

If you maintain a JAX-based library, your users are also JAX users. Know that JAX will continue to support “raw” old-style keys in [`jax.random`](../jax.random.html#module-jax.random "jax.random") for now, so callers may expect them to remain accepted everywhere. If you prefer to require new-style typed keys in your library, then you may want to enforce them with a check along the following lines:
...
### Customizing PRNG implementations[\#](#customizing-prng-implementations "Link to this heading")
...
### Safe PRNG key use[\#](#safe-prng-key-use "Link to this heading")
...
#### Key buffer indexing[\#](#key-buffer-indexing "Link to this heading")
...
#### Key arithmetic[\#](#key-arithmetic "Link to this heading")

Key arithmetic is a similarly treacherous way to derive keys from other keys. Deriving keys in a way that avoids [`jax.random.split()`](../_autosummary/jax.random.split.html#jax.random.split "jax.random.split") or [`jax.random.fold_in()`](../_autosummary/jax.random.fold_in.html#jax.random.fold_in "jax.random.fold_in") by manipulating key data directly produces a batch of keys that—depending on the PRNG implementation—might then generate correlated random numbers within the batch:
...
#### Inadvertent transposing of key buffers[\#](#inadvertent-transposing-of-key-buffers "Link to this heading")
...
#### Key reuse[\#](#key-reuse "Link to this heading")

Unlike state-based PRNG APIs like [`numpy.random`](https://numpy.org/doc/stable/reference/random/index.html#module-numpy.random "(in NumPy v2.4)"), JAX’s functional PRNG does not implicitly update a key when it has been used.
...
## Design of typed PRNG keys[\#](#design-of-typed-prng-keys "Link to this heading")
...
### Extended dtypes[\#](#extended-dtypes "Link to this heading")
...
Why introduce extended dtypes in generality, beyond PRNGs? We reuse this same extended dtype mechanism elsewhere internally. For example, the `jax._src.core.bint` object, a bounded integer type used for experimental work on dynamic shapes, is another extended dtype. In recent JAX versions it satisfies the properties above (See [jax/\_src/core.py#L1789-L1802](https://github.com/jax-ml/jax/blob/jax-v0.4.14/jax/_src/core.py#L1789-L1802)).

### PRNG dtypes[\#](#prng-dtypes "Link to this heading")
...
## Progress[\#](#progress "Link to this heading")

Following is a non-comprehensive list of key Pull Requests implementing the above design. The main tracking issue is [\#9263](https://github.com/jax-ml/jax/issues/9263).

- Implement pluggable PRNG via `PRNGImpl`: [\#6899](https://github.com/jax-ml/jax/issues/6899)

- Implement `PRNGKeyArray`, without dtype: [\#11952](https://github.com/jax-ml/jax/issues/11952)

- Add a “custom element” dtype property to `PRNGKeyArray` with `_rules` attribute: [\#12167](https://github.com/jax-ml/jax/issues/12167)

- Rename “custom element type” to “opaque dtype”: [\#12170](https://github.com/jax-ml/jax/issues/12170)

- Refactor `bint` to use the opaque dtype infrastructure: [\#12707](https://github.com/jax-ml/jax/issues/12707)

- Add `jax.random.key` to create typed keys directly: [\#16086](https://github.com/jax-ml/jax/issues/16086)

- Add `impl` argument to `key` and `PRNGKey`: [\#16589](https://github.com/jax-ml/jax/issues/16589)

- Rename “opaque dtype” to “extended dtype” & define `jax.dtypes.extended`: [\#16824](https://github.com/jax-ml/jax/issues/16824)

- Introduce `jax.dtypes.prng_key` and unify PRNG dtype with Extended dtype: [\#16781](https://github.com/jax-ml/jax/issues/16781)

- Add a `jax_legacy_prng_key` flag to support warning or erroring when using legacy (raw) PRNG keys: [\#17225](https://github.com/jax-ml/jax/issues/17225)

[](4410-omnistaging.html "previous page")

previous

Omnistaging

[](9407-type-promotion.html "next page")
