...
- [ .md](_sources/benchmarking.md "Download source file")
-  .pdf
...
1.  **JAX code is Just-In-Time (JIT) compiled.** Most code written in JAX can be written in such a way that it supports JIT compilation, which can make it run *much faster* (see [To JIT or not to JIT](https://docs.jax.dev/en/latest/notebooks/thinking_in_jax.html#to-jit-or-not-to-jit)). To get maximum performance from JAX, you should apply [`jax.jit()`](_autosummary/jax.jit.html#jax.jit "jax.jit") on your outer-most function calls.
...
2.  **JAX has asynchronous dispatch.** This means that you need to call `.block_until_ready()` to ensure that computation has actually happened (see [Asynchronous dispatch](async_dispatch.html#async-dispatch)).

3.  **JAX by default only uses 32-bit dtypes.** You may want to either explicitly use 32-bit dtypes in NumPy or enable 64-bit dtypes in JAX (see [Double (64 bit) precision](https://docs.jax.dev/en/latest/notebooks/Common_Gotchas_in_JAX.html#double-64bit-precision)) for a fair comparison.

4.  **Transferring data between CPUs and accelerators takes time.** If you only want to measure how long it takes to evaluate a function, you may want to transfer data to the device on which you want to run it first.

Here’s an example of how to put together all these tricks into a microbenchmark for comparing JAX versus NumPy, making using of IPython’s convenient [`%time` and `%timeit` magics](https://ipython.readthedocs.io/en/stable/interactive/magics.html#magic-time):
...
[](gpu_performance_tips.html "previous page")

previous

GPU performance tips

[](profiling.html "next page")
