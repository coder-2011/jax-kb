...
- [ .ipynb](../_sources/notebooks/host-offloading.ipynb "Download source file")
-  .pdf
...
# JAX Memories and Host Offloading[\#](#jax-memories-and-host-offloading "Link to this heading")
...
## Building Blocks for Offloading[\#](#building-blocks-for-offloading "Link to this heading")
...
### NamedSharding and Memory Kinds[\#](#namedsharding-and-memory-kinds "Link to this heading")

[`NamedSharding`](../jax.sharding.html#jax.sharding.NamedSharding "jax.sharding.NamedSharding") defines how data are distributed across devices. It includes:
...
### Data Placement with device_put[\#](#data-placement-with-device-put "Link to this heading")

[`jax.device_put()`](../_autosummary/jax.device_put.html#jax.device_put "jax.device_put") is a function that explicitly transfers arrays to a specified memory location according to a sharding specification.
...
### Output Sharding Controls[\#](#output-sharding-controls "Link to this heading")
...
#### Device Output Sharding[\#](#device-output-sharding "Link to this heading")
...
Moving data from host to device memory when needed for computation is the essence of host offloading. Use [`jax.device_put()`](../_autosummary/jax.device_put.html#jax.device_put "jax.device_put") to perform this transfer in this example to optimize performance.
...
#### Host Output Sharding[\#](#host-output-sharding "Link to this heading")
...
## Activation Offloading[\#](#activation-offloading "Link to this heading")
...
### Checkpoint Names[\#](#checkpoint-names "Link to this heading")

The `checkpoint_name()` function allows labeling activations for memory management during computation. Here’s a simple example that a checkpoint name `x` is specified.
...
[`jax.lax.scan()`](../_autosummary/jax.lax.scan.html#jax.lax.scan "jax.lax.scan") is commonly used in JAX for handling sequential operations (like RNNs or transformers). It can be integrated with JAX’s rematerialization to process sequential data.

Key components:

- `jax.remat()` creates a rematerialized version of the layer function using `jax.remat()` and applies the checkpoint policy to the layer function

- `prevent_cse=False` enables XLA’s common subexpression elimination for better performance

- [`jax.lax.scan()`](../_autosummary/jax.lax.scan.html#jax.lax.scan "jax.lax.scan") iterates the rematerialized layer along an axis
...
### Summary of Activation Offloading[\#](#summary-of-activation-offloading "Link to this heading")
...
## Parameter Offloading[\#](#parameter-offloading "Link to this heading")

Model parameters (also known as weights) can be offloaded to the host memory to optimize device memory usage during initialization. This is achieved by using [`jax.jit()`](../_autosummary/jax.jit.html#jax.jit "jax.jit") with a sharding strategy that specifies host memory kind.
...
### Parameter Placement for Computation[\#](#parameter-placement-for-computation "Link to this heading")

Different from the earlier `layer` function, [`jax.device_put()`](../_autosummary/jax.device_put.html#jax.device_put "jax.device_put") is applied to move parameter `w1` and `w2` to the device before the matrix multiplications. This ensures the parameters are available on the device for both forward and backward passes.
...
- `scanned` function combining `jax.remat()` and [`jax.lax.scan()`](../_autosummary/jax.lax.scan.html#jax.lax.scan "jax.lax.scan")

### Parameter Initialization with Host Offloading[\#](#parameter-initialization-with-host-offloading "Link to this heading")

During the initialization, parameter `w1` and `w2` are placed on host memory before being passed to the [`jax.jit()`](../_autosummary/jax.jit.html#jax.jit "jax.jit") function `f`, while keeping the `input` variable on the device.
...
### Memory Analysis[\#](#memory-analysis "Link to this heading")
...
### Limitations of Parameter Offloading[\#](#limitations-of-parameter-offloading "Link to this heading")

[`jax.lax.scan()`](../_autosummary/jax.lax.scan.html#jax.lax.scan "jax.lax.scan") is crucial for effective parameter management. Using an explicit for loop would cause parameters to continuously occupy device memory, resulting in the same memory usage as without parameter offloading. While [`jax.lax.scan()`](../_autosummary/jax.lax.scan.html#jax.lax.scan "jax.lax.scan") allows specifying the scan axis, parameter offloading currently works only when scanning over axis 0. Scanning over other axes generates a `transpose` operation during compilation before returning parameters to the device, which is expensive and not supported on all platforms.
...
# Optimizer State Offloading[\#](#optimizer-state-offloading "Link to this heading")
...
## Setting Up Sharding and Memory Kinds[\#](#setting-up-sharding-and-memory-kinds "Link to this heading")

`jax.sharding.SingleDeivceSharding()` is adopted to simplify the shardings for both device and host memory kinds. During the model state initialization, move the optimizer state to the host using `device_put()`.

## Model and Training Step Implementation[\#](#model-and-training-step-implementation "Link to this heading")

Next, define the model architecture, loss function, and training step. The key addition here is moving the optimizer state to device memory via `device_put()` at the beginning of each training step, as it’s needed for the parameter update on the device.

## Running and Comparing Results[\#](#running-and-comparing-results "Link to this heading")

After setting up the sharding, the optimizer state is moved to host memory and the step function is run with [`jax.jit()`](../_autosummary/jax.jit.html#jax.jit "jax.jit").
...
2.  Move optimizer states between host and device memory via [`jax.device_put()`](../_autosummary/jax.device_put.html#jax.device_put "jax.device_put")
...
1.  Argument Size Reduction:
    ...
    - The optimizer states are arguments of the [`jax.jit()`](../_autosummary/jax.jit.html#jax.jit "jax.jit") function
...
## Tools for Host Offloading[\#](#tools-for-host-offloading "Link to this heading")

[`jax.stages.Compiled.memory_analysis()`](../jax.stages.html#jax.stages.Compiled.memory_analysis "jax.stages.Compiled.memory_analysis") API is utilized above to get memory usage information. For device memory analysis, refer to [Profiling device memory](../device_memory_profiling.html). The profiling tools described in [Profiling computation](../profiling.html) can help measure memory savings and performance impact from host offloading.

[](layout.html "previous page")

previous

Device-local array layout control

[](../multi_process.html "next page")
