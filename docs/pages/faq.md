...
- [ .rst](_sources/faq.rst "Download source file")
-  .pdf
...
- [Why do some CUDA libraries fail to load/initialize?](#why-do-some-cuda-libraries-fail-to-load-initialize)
...
## `jit` changes the behavior of my function[\#](#jit-changes-the-behavior-of-my-function "Link to this heading")

If you have a Python function that changes behavior after using [`jax.jit()`](_autosummary/jax.jit.html#jax.jit "jax.jit"), perhaps your function uses global state, or has side-effects. In the following code, the `impure_func` uses the global `y` and has a side-effect due to `print`:
...
For [`jax.jit()`](_autosummary/jax.jit.html#jax.jit "jax.jit"), the function is executed once using the Python interpreter, at which time the `Inside` printing happens, and the first value of `y` is observed. Then, the function is compiled and cached, and executed multiple times with different values of `x`, but with the same first value of `y`.

Additional reading:

> - [JAX - The Sharp Bits](https://docs.jax.dev/en/latest/notebooks/Common_Gotchas_in_JAX.html)

## `jit` changes the exact numerics of outputs[\#](#jit-changes-the-exact-numerics-of-outputs "Link to this heading")
...
Unfortunately the full list of XLA’s algebraic simplifications is not well documented, but if you’re familiar with C++ and curious about what types of optimizations the XLA compiler makes, you can see them in the source code: [algebraic_simplifier.cc](https://github.com/openxla/xla/blob/33f815e190982dac4f20d1f35adb98497a382377/xla/hlo/transforms/simplifiers/algebraic_simplifier.cc#L4851).

## `jit` decorated function is very slow to compile[\#](#jit-decorated-function-is-very-slow-to-compile "Link to this heading")
...
This is usually a sign that calling your function generates a large amount of code in JAX’s internal representation, typically because it makes heavy use of Python control flow such as `for` loops. For a handful of loop iterations, Python is OK, but if you need *many* loop iterations, you should rewrite your code to make use of JAX’s [structured control flow primitives](https://docs.jax.dev/en/latest/control-flow.html#Structured-control-flow-primitives) (such as `lax.scan()`) or avoid wrapping the loop with `jit` (you can still use `jit` decorated functions *inside* the loop).

If you’re not sure if this is the problem, you can try running [`jax.make_jaxpr()`](_autosummary/jax.make_jaxpr.html#jax.make_jaxpr "jax.make_jaxpr") on your function. You can expect slow compilation if the output is many hundreds or thousands of lines long.

Sometimes it isn’t obvious how to rewrite your code to avoid Python loops because your code makes use of many arrays with different shapes. The recommended solution in this case is to make use of functions like [`jax.numpy.where()`](_autosummary/jax.numpy.where.html#jax.numpy.where "jax.numpy.where") to do your computation on padded arrays with fixed shape.

If your functions are slow to compile for another reason, please open an issue on GitHub.

## How to use `jit` with methods?[\#](#how-to-use-jit-with-methods "Link to this heading")

Moved to [🔪 Using jax.jit with class methods](notebooks/Common_Gotchas_in_JAX.html#jax-jit-class-methods).

## Is JAX faster than NumPy?[\#](#is-jax-faster-than-numpy "Link to this heading")
...
## Gradients contain NaN where using `where`[\#](#gradients-contain-nan-where-using-where "Link to this heading")
...
> - [Issue: gradients through jnp.where when one of branches is nan](https://github.com/jax-ml/jax/issues/1052#issuecomment-514083352).
>
> - [How to avoid NaN gradients when using where](https://github.com/tensorflow/probability/blob/master/discussion/where-nan.pdf).

## Why are gradients zero for functions based on sort order?[\#](#why-are-gradients-zero-for-functions-based-on-sort-order "Link to this heading")
...
The problem stems from the fact that our function has a discontinuity at `x`` ``=`` ``0`. Our `f` here is essentially a [Heaviside Step Function](https://en.wikipedia.org/wiki/Heaviside_step_function), and we can use a [Sigmoid Function](https://en.wikipedia.org/wiki/Sigmoid_function) as a smoothed replacement. The sigmoid is approximately equal to the heaviside function when x is far from zero, but replaces the discontinuity at `x`` ``=`` ``0` with a smooth, differentiable curve. As a result of using [`jax.nn.sigmoid()`](_autosummary/jax.nn.sigmoid.html#jax.nn.sigmoid "jax.nn.sigmoid"), we get a similar computation with well-defined gradients:
...
The [`jax.nn`](jax.nn.html#module-jax.nn "jax.nn") submodule also has smooth versions of other common rank-based functions, for example [`jax.nn.softmax()`](_autosummary/jax.nn.softmax.html#jax.nn.softmax "jax.nn.softmax") can replace uses of [`jax.numpy.argmax()`](_autosummary/jax.numpy.argmax.html#jax.numpy.argmax "jax.numpy.argmax"), [`jax.nn.soft_sign()`](_autosummary/jax.nn.soft_sign.html#jax.nn.soft_sign "jax.nn.soft_sign") can replace uses of [`jax.numpy.sign()`](_autosummary/jax.numpy.sign.html#jax.numpy.sign "jax.numpy.sign"), [`jax.nn.softplus()`](_autosummary/jax.nn.softplus.html#jax.nn.softplus "jax.nn.softplus") or [`jax.nn.squareplus()`](_autosummary/jax.nn.squareplus.html#jax.nn.squareplus "jax.nn.squareplus") can replace uses of [`jax.nn.relu()`](_autosummary/jax.nn.relu.html#jax.nn.relu "jax.nn.relu"), etc.

## How can I convert a JAX Tracer to a NumPy array?[\#](#how-can-i-convert-a-jax-tracer-to-a-numpy-array "Link to this heading")
...
- If you wish to print a traced value at runtime for debugging purposes, you might consider using [`jax.debug.print()`](_autosummary/jax.debug.print.html#jax.debug.print "jax.debug.print").

- If you wish to call non-JAX code within a transformed JAX function, you might consider using [`jax.pure_callback()`](_autosummary/jax.pure_callback.html#jax.pure_callback "jax.pure_callback"), an example of which is available at [Pure callback example](https://docs.jax.dev/en/latest/notebooks/external_callbacks.html#example-pure-callback-with-custom-jvp).

- If you wish to input or output array buffers at runtime (for example, load data from file, or log the contents of the array to disk), you might consider using [`jax.experimental.io_callback()`](_autosummary/jax.experimental.io_callback.html#jax.experimental.io_callback "jax.experimental.io_callback"), an example of which can be found at [IO callback example](https://docs.jax.dev/en/latest/notebooks/external_callbacks.html#exploring-jax-experimental-io-callback).

For more information on runtime callbacks and examples of their use, see [External callbacks in JAX](https://docs.jax.dev/en/latest/notebooks/external_callbacks.html).

## Why do some CUDA libraries fail to load/initialize?[\#](#why-do-some-cuda-libraries-fail-to-load-initialize "Link to this heading")

When resolving dynamic libraries, JAX uses the usual [dynamic linker search pattern](https://man7.org/linux/man-pages/man8/ld.so.8.html). JAX sets `RPATH` to point to the JAX-relative location of the pip-installed NVIDIA CUDA packages, preferring them if installed. If `ld.so` cannot find your CUDA runtime libraries along its usual search path, then you must include the paths to those libraries explicitly in `LD_LIBRARY_PATH`. The easiest way to ensure your CUDA files are discoverable is to simply install the `nvidia-*-cu12` pip packages, which are included in the standard `jax[cuda_12]` install option.
