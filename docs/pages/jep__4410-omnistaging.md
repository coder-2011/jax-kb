...
- [ .md](../_sources/jep/4410-omnistaging.md "Download source file")
-  .pdf
...
# Omnistaging[\#](#omnistaging "Link to this heading")
...
A change to JAX’s tracing infrastructure called “omnistaging” ([jax-ml/jax#3370](https://github.com/jax-ml/jax/pull/3370)) was switched on in jax==0.2.0. This change improves memory performance, trace execution time, and simplifies jax internals, but may cause some existing code to break. Breakage is usually a result of buggy code, so long-term it’s best to fix the bugs, but omnistaging can also be disabled as a temporary workaround. And we’re happy to help you with fixes!

### How do I know if omnistaging broke my code?[\#](#how-do-i-know-if-omnistaging-broke-my-code "Link to this heading")
...
### How can I disable omnistaging for now?[\#](#how-can-i-disable-omnistaging-for-now "Link to this heading")
...
### How do I fix bugs exposed by omnistaging?[\#](#how-do-i-fix-bugs-exposed-by-omnistaging "Link to this heading")
...
## What is “omnistaging” and why is it useful?[\#](#what-is-omnistaging-and-why-is-it-useful "Link to this heading")
...
### Slightly less toy example[\#](#slightly-less-toy-example "Link to this heading")
...
(The `broadcast` that corresponds to the construction of the zeros array for `jnp.zeros_like(x)` is staged out because JAX is lazy about very simple expressions from [jax-ml/jax#1668](https://github.com/jax-ml/jax/pull/1668). After omnistaging, we can remove that lazy sublanguage and simplify JAX internals.)
...
## What issues can arise when omnistaging is switched on?[\#](#what-issues-can-arise-when-omnistaging-is-switched-on "Link to this heading")
...
### Using `jax.numpy` for shape computations[\#](#using-jax-numpy-for-shape-computations "Link to this heading")
...
### Side-effects[\#](#side-effects "Link to this heading")
...
#### Error message[\#](#id2 "Link to this heading")
...
#### Explanation[\#](#id3 "Link to this heading")
...
#### Solution[\#](#id4 "Link to this heading")

The solution is to identify JAX-transformed functions that rely on side effects, and to rewrite them not to be effectful.

### Small numerical differences based on XLA optimizations[\#](#small-numerical-differences-based-on-xla-optimizations "Link to this heading")
...
### Dependence on JAX internal APIs that changed[\#](#dependence-on-jax-internal-apis-that-changed "Link to this heading")
...
### Triggering XLA compile time bugs[\#](#triggering-xla-compile-time-bugs "Link to this heading")
...
[](4008-custom-vjp-update.html "previous page")

previous

`custom_vjp` and `nondiff_argnums` update guide

[](9263-typed-keys.html "next page")
