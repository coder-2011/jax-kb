...
- [ .md](_sources/complex-differentiation.md "Download source file")
-  .pdf
...
What about convenience wrappers like [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad"), [`jax.jacfwd()`](_autosummary/jax.jacfwd.html#jax.jacfwd "jax.jacfwd"), and [`jax.jacrev()`](_autosummary/jax.jacrev.html#jax.jacrev "jax.jacrev")?
...
Because this only works for holomorphic functions, to use this trick we need to promise JAX that our function is holomorphic; otherwise, JAX will raise an error when [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") is used for a complex-output function:
...
There are some useful upshots for how [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") works here:

1.  We can use [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") on holomorphic \\\mathbb{C} \to \mathbb{C}\\ functions.

2.  We can use [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") to optimize \\f : \mathbb{C} \to \mathbb{R}\\ functions, like real-valued loss functions of complex parameters `x`, by taking steps in the direction of the conjugate of `grad(f)(x)`.

3.  If we have an \\\mathbb{R} \to \mathbb{R}\\ function that just happens to use some complex-valued operations internally (some of which must be non-holomorphic, e.g. FFTs used in convolutions) then [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") still works and we get the same result that an implementation using only real values would have given.
...
[](jacobian-vector-products.html "previous page")

previous

Forward- and reverse-mode autodiff in JAX

[](notebooks/Custom_derivative_rules_for_Python_code.html "next page")
