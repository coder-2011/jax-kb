...
- [ .md](_sources/aot.md "Download source file")
-  .pdf
...
- [Tracing with static arguments](#tracing-with-static-arguments)
...
# Ahead-of-time lowering and compilation[\#](#ahead-of-time-lowering-and-compilation "Link to this heading")
...
Note that the lowered objects can be used only in the same process in which they were lowered. For exporting use cases, see the [Exporting and serialization](export/index.html#export) APIs.

See the [`jax.stages`](jax.stages.html#module-jax.stages "jax.stages") documentation for more details on what functionality the lowering and compiled functions provide.
...
Relatedly, AOT-compiled functions [cannot be transformed by JAX’s just-in-time transformations](#aot-compiled-functions-cannot-be-transformed) such as `jax.jit`, [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad"), and [`jax.vmap()`](_autosummary/jax.vmap.html#jax.vmap "jax.vmap").

## Tracing with static arguments[\#](#tracing-with-static-arguments "Link to this heading")
...
The results of `trace` and of `lower` are not safe to serialize directly for use in a different process. See [Exporting and serialization](export/index.html#export) for additional APIs for this purpose.

## AOT-compiled functions cannot be transformed[\#](#aot-compiled-functions-cannot-be-transformed "Link to this heading")

Compiled functions are specialized to a particular set of argument “types,” such as arrays with a specific shape and element type in our running example. From JAX’s internal point of view, transformations such as [`jax.vmap()`](_autosummary/jax.vmap.html#jax.vmap "jax.vmap") alter the type signature of functions in a way that invalidates the compiled-for type signature. As a policy, JAX simply disallows compiled functions to be involved in transformations. Example:
...
A similar error is raised when `g_aot` is involved in autodiff (e.g. [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad")). For consistency, transformation by `jax.jit` is disallowed as well, even though `jit` does not meaningfully modify its argument’s type signature.

## Debug information and analyses, when available[\#](#debug-information-and-analyses-when-available "Link to this heading")
...
For instance, as the initial example above shows, lowered functions often offer a text representation. Compiled functions do the same, and also offer cost and memory analyses from the compiler. All of these are provided via methods on the [`jax.stages.Lowered`](jax.stages.html#jax.stages.Lowered "jax.stages.Lowered") and [`jax.stages.Compiled`](jax.stages.html#jax.stages.Compiled "jax.stages.Compiled") objects (e.g., `lowered.as_text()` and `compiled.cost_analysis()` above). You can obtain more debugging information, e.g., source location, by using the `debug_info` parameter to `lowered.as_text()`.
...
When in doubt, see the package API documentation for [`jax.stages`](jax.stages.html#module-jax.stages "jax.stages").

[](gradient-checkpointing.html "previous page")

previous

Gradient checkpointing with `jax.checkpoint` (`jax.remat`)

[](export/index.html "next page")
