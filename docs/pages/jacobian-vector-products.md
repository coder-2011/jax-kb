...
- [ .md](_sources/jacobian-vector-products.md "Download source file")
-  .pdf
...
- [Jacobian-Vector products (JVPs, a.k.a. forward-mode autodiff)](#jacobian-vector-products-jvps-a-k-a-forward-mode-autodiff)
  - [JVPs in math](#jvps-in-math)
  - [JVPs in JAX code](#jvps-in-jax-code)
- [Vector-Jacobian products (VJPs, a.k.a. reverse-mode autodiff)](#vector-jacobian-products-vjps-a-k-a-reverse-mode-autodiff)
...
- [Composing VJPs, JVPs, and `jax.vmap`](#composing-vjps-jvps-and-jax-vmap)
- [Jacobian-Matrix and Matrix-Jacobian products](#jacobian-matrix-and-matrix-jacobian-products)
- [The implementation of `jax.jacfwd` and `jax.jacrev`](#the-implementation-of-jax-jacfwd-and-jax-jacrev)

# Forward- and reverse-mode autodiff in JAX[\#](#forward-and-reverse-mode-autodiff-in-jax "Link to this heading")

## Jacobian-Vector products (JVPs, a.k.a. forward-mode autodiff)[\#](#jacobian-vector-products-jvps-a-k-a-forward-mode-autodiff "Link to this heading")

JAX includes efficient and general implementations of both forward- and reverse-mode automatic differentiation. The familiar [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") function is built on reverse-mode, but to explain the difference between the two modes, and when each can be useful, you need a bit of math background.
...
Back in Python code, JAX’s [`jax.jvp()`](_autosummary/jax.jvp.html#jax.jvp "jax.jvp") function models this transformation. Given a Python function that evaluates \\f\\, JAX’s [`jax.jvp()`](_autosummary/jax.jvp.html#jax.jvp "jax.jvp") is a way to get a Python function for evaluating \\(x, v) \mapsto (f(x), \partial f(x) v)\\.
...
## Vector-Jacobian products (VJPs, a.k.a. reverse-mode autodiff)[\#](#vector-jacobian-products-vjps-a-k-a-reverse-mode-autodiff "Link to this heading")
...
This is great because it lets us build Jacobian matrices one row at a time, and the FLOP cost for evaluating \\(x, v) \mapsto (f(x), v^\mathsf{T} \partial f(x))\\ is only about three times the cost of evaluating \\f\\. In particular, if we want the gradient of a function \\f : \mathbb{R}^n \to \mathbb{R}\\, we can do it in just one call. That’s how [`jax.grad()`](_autosummary/jax.grad.html#jax.grad "jax.grad") is efficient for gradient-based optimization, even for objectives like neural network training loss functions on millions or billions of parameters.
...
Even better, since you didn’t have to call `jnp.dot()` directly, this `hvp` function works with arrays of any shape and with arbitrary container types (like vectors stored as nested lists/dicts/tuples), and doesn’t even have a dependence on [`jax.numpy`](jax.numpy.html#module-jax.numpy "jax.numpy").
...
That’s not quite as good, though, because forward-mode has less overhead than reverse-mode, and since the outer differentiation operator here has to differentiate a larger computation than the inner one, keeping forward-mode on the outside works best:
    ...
    Forward over reverse
    4.44 ms ± 159 μs per loop (mean ± std. dev. of 3 runs, 10 loops each)
    Reverse over forward
    The slowest run took 4.29 times longer than the fastest. This could mean that an intermediate result is being cached.
    13.9 ms ± 10.3 ms per loop (mean ± std. dev. of 3 runs, 10 loops each)
    Reverse over reverse
    15.8 ms ± 7.5 ms per loop (mean ± std. dev. of 3 runs, 10 loops each)
    Naive full Hessian materialization
    60.4 ms ± 2.93 ms per loop (mean ± std. dev. of 3 runs, 10 loops each)

## Composing VJPs, JVPs, and `jax.vmap`[\#](#composing-vjps-jvps-and-jax-vmap "Link to this heading")

## Jacobian-Matrix and Matrix-Jacobian products[\#](#jacobian-matrix-and-matrix-jacobian-products "Link to this heading")

Now that you have [`jax.jvp()`](_autosummary/jax.jvp.html#jax.jvp "jax.jvp") and [`jax.vjp()`](_autosummary/jax.vjp.html#jax.vjp "jax.vjp") transformations that give you functions to push-forward or pull-back single vectors at a time, you can use JAX’s [`jax.vmap()`](_autosummary/jax.vmap.html#jax.vmap "jax.vmap") [transformation](https://github.com/jax-ml/jax#auto-vectorization-with-vmap) to push and pull entire bases at once. In particular, you can use that to write fast matrix-Jacobian and Jacobian-matrix products:
    ...
    119 ms ± 251 μs per loop (mean ± std. dev. of 3 runs, 10 loops each)

    Vmapped Matrix-Jacobian product
    4.92 ms ± 168 μs per loop (mean ± std. dev. of 3 runs, 10 loops each)
    ...
    130 ms ± 519 μs per loop (mean ± std. dev. of 3 runs, 10 loops each)

    Vmapped Jacobian-Matrix product
    1.88 ms ± 74.7 μs per loop (mean ± std. dev. of 3 runs, 10 loops each)

## The implementation of `jax.jacfwd` and `jax.jacrev`[\#](#the-implementation-of-jax-jacfwd-and-jax-jacrev "Link to this heading")

Now that we’ve seen fast Jacobian-matrix and matrix-Jacobian products, it’s not hard to guess how to write [`jax.jacfwd()`](_autosummary/jax.jacfwd.html#jax.jacfwd "jax.jacfwd") and [`jax.jacrev()`](_autosummary/jax.jacrev.html#jax.jacrev "jax.jacrev"). We just use the same technique to push-forward or pull-back an entire standard basis (isomorphic to an identity matrix) at once.
...
Interestingly, the [Autograd](https://github.com/hips/autograd) library couldn’t do this. The [implementation](https://github.com/HIPS/autograd/blob/96a03f44da43cd7044c61ac945c483955deba957/autograd/differential_operators.py#L60) of reverse-mode `jacobian` in Autograd had to pull back one vector at a time with an outer-loop `map`. Pushing one vector at a time through the computation is much less efficient than batching it all together with [`jax.vmap()`](_autosummary/jax.vmap.html#jax.vmap "jax.vmap").

Another thing that Autograd couldn’t do is [`jax.jit()`](_autosummary/jax.jit.html#jax.jit "jax.jit"). Interestingly, no matter how much Python dynamism you use in your function to be differentiated, we could always use [`jax.jit()`](_autosummary/jax.jit.html#jax.jit "jax.jit") on the linear part of the computation. For example:
...
[](higher-order.html "previous page")

previous

Higher-order derivatives

[](complex-differentiation.html "next page")
