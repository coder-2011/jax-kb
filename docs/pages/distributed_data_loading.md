...
- [ .md](_sources/distributed_data_loading.md "Download source file")
-  .pdf
...
- [General approach for loading a `jax.Array`](#general-approach-for-loading-a-jax-array)
  - [Examples](#examples)
...
# Distributed data loading[\#](#distributed-data-loading "Link to this heading")
...
## General approach for loading a `jax.Array`[\#](#general-approach-for-loading-a-jax-array "Link to this heading")

Consider a case of creating a single [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") from raw data not produced by JAX. These concepts apply beyond loading batched data records, such as any multi-process [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") that wasn’t directly produced by a JAX computation. Examples include: 1) loading model weights from a checkpoint; or 2) loading a large spatially-sharded image.

Every [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") has an associated [`Sharding`](jax.sharding.html#jax.sharding.Sharding "jax.sharding.Sharding"), which describes which shard of the global data is required by each global device. When you create a [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") from scratch, you also need to create its `Sharding`. This is how JAX can understand how the data is laid out across devices. You can create whatever `Sharding` you want. In practice, you usually pick a `Sharding` based on what kind of parallelism strategy you are implementing (you will learn more about data and model parallelism in more detail later in this guide). You can also pick a `Sharding` based on how the raw data will be produced within each process.

Once you have defined a [`Sharding`](jax.sharding.html#jax.sharding.Sharding "jax.sharding.Sharding"), you can use [`addressable_devices()`](jax.sharding.html#jax.sharding.Sharding.addressable_devices "jax.sharding.Sharding.addressable_devices") to provide a list of devices needed to load data for within the current process. (Note: The term “addressable devices” is a more general version of “local devices”. The goal is to make sure that each process’s data loader provides the right data to all of that process’ local devices.

### Examples[\#](#examples "Link to this heading")

For example, consider a `(64,`` ``128)` [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") that you need to shard across 4 processes with 2 devices each (8 devices total). This will result in 8 unique data shards, one for each device. There are many ways to shard this [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"). You can perform a 1D sharding across the second dimension of the [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"), giving each device a `(64,`` ``16)` shard, as demonstrated below:

![8 unique data shards](_images/1.svg)
...
### Option 2: Use a per-device data pipeline[\#](#option-2-use-a-per-device-data-pipeline "Link to this heading")

![Using a per-device data pipeline](_images/5.svg)
...
### Option 3: Use a consolidated per-process data pipeline[\#](#option-3-use-a-consolidated-per-process-data-pipeline "Link to this heading")

![Using a consolidated per-process data pipeline](_images/6.svg)
...
### Option 4: Load data in some convenient way, reshard inside computation[\#](#option-4-load-data-in-some-convenient-way-reshard-inside-computation "Link to this heading")

![Loading data in some convenient way, reshard inside computation](_images/7.svg)
...
Then, you can create a [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") with a [`Sharding`](jax.sharding.html#jax.sharding.Sharding "jax.sharding.Sharding") representing the per-column data, pass that directly into the computation, and use [`jax.lax.with_sharding_constraint()`](_autosummary/jax.lax.with_sharding_constraint.html#jax.lax.with_sharding_constraint "jax.lax.with_sharding_constraint") to immediately reshard the column-sharded input to the desired sharding. And since the data is resharded inside the computation, it will be resharded over the accelerator communication links (for example, TPU ICI or NVLink).
...
### Full replication[\#](#full-replication "Link to this heading")
...
![Full replication](_images/8.svg)

### Partial replication[\#](#partial-replication "Link to this heading")

**Partial replication** describes a process where there are multiple copies of the data, and each copy is sharded across multiple devices. For a given array value, there are generally many possible ways to perform partial replication (Note: There is always a single fully-replicated [`Sharding`](jax.sharding.html#jax.sharding.Sharding "jax.sharding.Sharding") for a given array shape).
...
![Partial replication - example 1](_images/9.svg)
...
![Partial replication - example 2](_images/10.svg)

Now that you’ve gone over the high-level options for creating a [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"), let’s apply them to data loading for ML applications.
...
![Data parallelism - example 1](_images/11.svg)

When representing the input data as a single [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"), the Array contains the data across all replicas for this step (this is called *global batch*), with each shard of the [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") containing a single per-replica batch. You can represent this as a 1D sharding across all devices (check the example below) — in other words, the global batch is composed of all the per-replica batches concatenated together across the batch axis.

![Data parallelism - example 2](_images/12.svg)
...
## Important trick about data parallelism[\#](#important-trick-about-data-parallelism "Link to this heading")
...
![Data parallelism - example 3](_images/13.svg)

Usually, rearranging the data shards of a [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"), as demonstrated above, is not a good idea – you’re effectively permuting the value of the [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array")! However, for data parallelism, the global batch order isn’t meaningful, and you are free to rearrange the per-replica batches in the global batch, as already mentioned before.
...
![Data parallelism - example 4](_images/14.svg)
...
## Data + model parallelism[\#](#data-model-parallelism "Link to this heading")
...
### Model parallelism within a process[\#](#model-parallelism-within-a-process "Link to this heading")
...
![Data and model parallelism - example 1](_images/15.svg)

Here, once again, the input data is represented as a single [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") with a 1D sharding where each shard is a per-replica batch with an exception:
...
![Data and model parallelism - example 2](_images/16.svg)
...
![Data and model parallelism - example 3](_images/17.svg)
...
![Data and model parallelism - example 4](_images/18.svg)

However, if you’re not careful about which local device you load each batch onto, you may accidentally create unreplicated data, even though the [`Sharding`](jax.sharding.html#jax.sharding.Sharding "jax.sharding.Sharding") (and the parallelism strategy) says the data is replicated:

![Data and model parallelism - example 4](_images/19.svg)

JAX will raise an error if you accidentally create a [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") with unreplicated data that should be replicated within a single process (this isn’t always true for model parallelism across processes though; see the next section).
...
### Model parallelism across processes[\#](#model-parallelism-across-processes "Link to this heading")
...
![Model parallelism across processes - example 1](_images/20.svg)
...
![Model parallelism across processes - example 2](_images/21.svg)

Note

As of August 2023, JAX cannot detect if [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array") shards across processes are supposed to be replicated but aren’t, and will produce wrong results when the computation is run. So be careful not to do this!

To get the correct per-replica batch on each device, you need to represent the global input data as the following [`jax.Array`](_autosummary/jax.Array.html#jax.Array "jax.Array"):

![Model parallelism across processes - example 3](_images/22.svg)

[](fault_tolerance.html "previous page")

previous

Fault Tolerant Distributed JAX

[](notebooks/colocated-python.html "next page")
